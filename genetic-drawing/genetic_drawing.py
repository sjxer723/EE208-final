...
#Copy from https://github.com/anopara/genetic-drawing
import cv2
import numpy as np
import time
import os
...
class AI_painter(object):

    def __init__(self,__filename):
        self.filename=__filename

    def generate_pic(self):
        #load the example image and set the generator for 50 stages with 10 generations each
        gen=GeneticDrawing('../static/image/'+self.filename,seed=time.time())
        out=gen.generate(50,10)
        ...
        for i in range(len(gen.imgBuffer)):
            ...
            img_paths.append(os.path.join("out", f"{i:06d}.png"))
        gif_images=[]
        for path in img_paths:
            gif_images.append(imageio.imread(path))
        imageio.mimsave("../static/result.gif",gif_images,fps=1)

if __name__ == "__main__":
    ai=AI_painter('httpwww.jhzhx.comgjzq27.html%0.jpg')
    ai.generate_pic()